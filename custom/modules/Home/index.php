...
global $current_user, $sugar_version, $sugar_config, $beanFiles, $theme;
...
// Check if we should display Real Estate Hub or regular dashboard
$dashboardType = $current_user->getPreference('dashboard_type', 'Home');

// Default to Real Estate Hub if no preference is set
...
if ($dashboardType === 'real_estate_hub') {
    // Display Real Estate Hub
    require_once('custom/modules/Home/views/view.realestatehub.php');
    $view = new HomeViewRealEstateHub();
    $view->display();
} else {
    // Include the original dashboard code directly to avoid circular reference
    ...
    // Add dashboard type tabs before rendering
    ...
    // Use the original template for SuiteCRM dashboard
